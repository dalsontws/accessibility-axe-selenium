...
p = figure(
    x_range=urlno,
    plot_height=350,
    title="Violation Count by Website",
    toolbar_location="above",
    tools="hover,pan,wheel_zoom,reset,save",
    tooltips="@$name $name issue(s) found",
)

p.vbar_stack(
    impact, x="urlno", width=0.9, color=colors, source=data, legend_label=impact
)
...
data = (
    pd.Series(piechart).reset_index(name="value").rename(columns={"index": "country"})
)
...
violationstable.sort_values(
    by=["Most Common Violation Types"], inplace=True, ascending=False
)
